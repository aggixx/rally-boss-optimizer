...
class Hand:
	...
	def get_score(self):
		if repr(self.cards) in self.app.hand_cache:
			return self.app.hand_cache[repr(self.cards)]
		...
		score = mean(hand_resources)
		self.app.hand_cache[repr(self.cards)] = score
...
class Deck:
	...
	def get_score(self):
		if not hasattr(self, 'score'):
			...
			self.score = mean(deck_resources)
			logging.debug("Deck average: {}".format(self.score))

		return self.score

class Card:
	def __init__(self, elements, resource_element, resource_amount):
		self.elements = elements
		self.resource_amount = resource_amount
		self.resource_element = resource_element

	def __str__(self):
		return "Card-{}-{}{}".format("".join(map(lambda e: e.short(), self.elements)), self.resource_element.short(), self.resource_amount)
...
class Boss:
	...
	def calculateResources(self, card):
		...
		logging.debug("Resource amount: {} {}".format(amount, card.resource_element))
	...
	def load(self, path):
		...
		with open(path, 'r') as f:
			...
			for line in raw_cards:
				...
				resource_element = Resource[fields[1][:1]]
				resource_amount = int(fields[1][1:2])
				logging.debug("Resource: {} {}".format(resource_amount, resource_element))

				cards.append(Card(elements, resource_element, resource_amount))
